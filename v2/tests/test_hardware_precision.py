...
class TestYamlAndFactory(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        doc = config_loader._load_yaml(hw_path)
        cls.hw_cfg = config_loader._parse_hardware(doc)
    ...
    def test_yaml_multi_precision_derived(self):
        ...
        # 例如 GPU 不支持 INT4 execution（即使配置层声明了），也不支持 NONLINEAR 类别。
        self.assertFalse(gpu.can_execute(_mk_op(
            OC.NONLINEAR, PL.FP16, PL.FP32)))     # GPU 无 NONLINEAR 类别
