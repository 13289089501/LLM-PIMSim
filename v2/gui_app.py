...
from model_lib import get_model, list_models
...
def api_experiment_create():
    """新建实验：{name, model, clone_from} → 克隆模板生成一组独立实验文件。
    clone_from 为现有实验入口（默认 01_gpu_only.yaml）。"""
    ...
    # 读取模板实验入口，克隆其子配置结构
    clone_from = (d.get("clone_from") or "experiments/01_gpu_only.yaml").strip()
    src_entry = BASE/"configs"/clone_from
    files, src_base, seed = {}, exps_dir, 42
    try:
        src_doc = yaml.safe_load(src_entry.read_text(encoding="utf-8"))
        files = ((src_doc or {}).get("experiment", {}) or {}).get("files", {}) or {}
        src_base = src_entry.parent
        seed = ((src_doc or {}).get("experiment", {}) or {}).get("seed", 42)
    except Exception:
        pass

    # 复制模板子文件为 <实验>_<key>.yaml（平铺在 experiments/，与现有结构一致）
    new_files = {}
...
def api_recommend():
    ...
    d = request.get_json() or {}
    exp_path = (d.get("experiment") or "experiments/01_gpu_only.yaml").strip()
...
def _recommend_devices(exp_path, ops_in, hw_in):
    """计算每个算子的推荐设备（backId）。IC 参考策略（集成电路视角）：
      ...
      - 归约/逐元素（ln、softmax、silu）→ SRAM-PIM（带宽高、FP32 精度稳）
      - resid/kv_update → 就近（resid1→GPU, resid2/kv→PIM）
    若推荐设备不在画布，退化为按效率选最优可用设备。"""
    hw_ids = set()
    for h in hw_in:
        hw_ids.add(h.get("id"))
        if h.get("backId"):
    ...
    for o in ops_in:
        ...
        if not oid:
            continue
        dev = _ic_dev(oid, o.get("op_type") or "GEMM")
        if dev is None:
            # 推荐的设备类型画布上没有 → 按效率选最优可用设备
            alt = _best_device_by_eff({"op_type": o.get("op_type") or "GEMM",
                                       "precision": o.get("precision") or "FP16"}, hw_in)
...
def api_validate():
    """配置约束校验：{hardware, operators, connections, compute_map, weight_blocks} → 校验结果"""
    d = request.get_json() or {}
    vr = validate_config(d.get("hardware"), d.get("operators"),
                         d.get("connections"), d.get("compute_map"),
                         d.get("weight_blocks"))
...
def api_run():
    ...
    exp_path = d.get("experiment","experiments/01_gpu_only.yaml")
...
def api_split():
    ...
    from workload_model import split_kernel_dict
    d = request.get_json()
...
<title>LLM-PIMSim — 可视化拓扑编辑器</title>
<style>
:root{--bg:#1a1d23;--panel:#21252b;--text:#abb2bf;--text2:#5c6370;--accent:#61afef;
  --green:#98c379;--orange:#d19a66;--purple:#c678dd;--red:#e06c75;--border:#333842;
...
/* toolbar */
.toolbar{width:240px;background:var(--panel);border-right:1px solid var(--border);display:flex;flex-direction:column;flex-shrink:0;overflow-y:auto}
.toolbar h2{font-size:14px;padding:14px 16px 8px;color:#fff;font-weight:600}
...
.toolbar button{width:100%;padding:8px 12px;border:none;border-radius:4px;font-size:12px;font-weight:600;cursor:pointer;margin:3px 0;transition:all .12s}
.btn-add{background:#2c313a;color:var(--text);border:1px dashed var(--border)}
.btn-add:hover{background:#3a4050;border-color:var(--accent);color:var(--accent)}
.btn-run{background:var(--accent);color:#1a1d23}
.btn-run:hover{background:#7ec8ff}
.btn-save{background:#2c313a;color:var(--green);border:1px solid #3a5040}
.btn-save:hover{background:#3a5040}
.btn-validate{background:#2c313a;color:var(--orange);border:1px solid #5a4630}
.btn-validate:hover{background:#5a4630}
/* canvas */
...
/* blocks */
.block{position:absolute;border-radius:6px;cursor:grab;user-select:none;z-index:2;transition:box-shadow .15s}
.block:hover{z-index:10}
.block.dragging{box-shadow:0 8px 32px rgba(0,0,0,.5);z-index:100;cursor:grabbing;opacity:.9}
.block.selected{box-shadow:0 0 0 2px var(--accent),0 4px 16px rgba(0,0,0,.4)}
.hw-block{min-width:220px;max-width:300px;border:2px solid #555}
.hw-block .hw-header{padding:8px 12px;border-radius:4px 4px 0 0;font-weight:700;font-size:12px;color:#1a1d23}
.hw-block .hw-body{padding:8px 12px;background:#252931;border-radius:0 0 4px 4px;font-size:11px}
...
.op-block{min-width:280px;background:#2c313a;border:1px solid #4a505a;padding:10px 12px;font-size:11px}
.op-block .op-row{display:flex;justify-content:space-between;gap:8px;padding:2.5px 0;border-bottom:1px solid #2a2f38;align-items:baseline}
.op-block .op-row:last-of-type{border-bottom:none}
.op-block .op-row>span:first-child{color:var(--text2);flex-shrink:0}
.op-block .op-row .val{color:#e5e5e5;text-align:right;word-break:break-all}
...
/* weight blocks */
.w-block{min-width:210px;background:#2a2438;border:1px solid #6a5acd;padding:8px 12px;font-size:11px;z-index:2}
...
<div class="toolbar" id="toolbar">
  ...
  <div id="new-exp-overlay" class="modal-overlay">
    <div class="modal drag-modal" id="new-exp-modal">
      ...
      <label style="display:block;font-size:11px;color:var(--text2);margin:10px 0 3px">克隆自（复用其硬件/互连/映射/放置模板）</label>
...
function loadModel(){
  ...
  fetch('/api/workload?model='+m).then(r=>r.json()).then(wl=>{
    ...
    kernels.forEach((k,i)=>{
      ...
      el.innerHTML=`
        <div class="op-row"><span>类型</span><b>${k.op_type||''}</b></div>
...
function loadWeights(){
  ...
  wCounter=0;
  return fetch('/api/weights?model='+m+'&experiment='+encodeURIComponent(exp||'experiments/01_gpu_only.yaml'))
...
function splitWeight(wid){
  ...
  let m=document.getElementById('sel-model').value;
  let exp=document.getElementById('sel-experiment')?document.getElementById('sel-experiment').value:'';
  fetch('/api/weights?model='+m+'&experiment='+encodeURIComponent(exp||'experiments/01_gpu_only.yaml')+'&split='+cls+':'+n)
  .then(r=>r.json()).then(d=>{
...
function applyRecommendation(){
  ...
  fetch('/api/reference',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({experiment:exp||'experiments/01_gpu_only.yaml', operators:operators, hardware:hardware,
...
function validateSim(){
  ...
  fetch('/api/validate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(st)})
...
function runSim(){
  ...
  fetch('/api/run',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({experiment:exp, compute_map:st.run_map, state:st, run_validation:runValidation})})
  .then(r=>r.json()).then(d=>{
    ...
    showResult('仿真结果', `
      ...
      <div class="metric"><span>算子数</span><span class="v">${d.op_count||0}</span></div>
      <div style="color:var(--text2);font-size:10px;margin-top:8px">${d.rationale||''}</div>
      <div style="color:var(--green);font-size:10px;margin-top:6px">${mapNote}${d.override_applied>=0?' · 后端覆盖'+d.override_applied+'个算子':''}</div>
    `);
...
function openNewExperiment(){
  ...
  fetch('/api/experiments').then(r=>r.json()).then(d=>{
    ...
    if(curVal && list.some(e=>e.path===curVal)) csel.value=curVal;
  });
...
function createExperiment(){
  ...
  let clone=document.getElementById('new-exp-clone').value||'experiments/01_gpu_only.yaml';
  ...
  fetch('/api/experiment/create',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({name:name, model:model, clone_from:clone})})
  .then(r=>r.json()).then(d=>{
...
function doSplit(){
  ...
  .then(r=>r.json()).then(d=>{
    ...
    closeSplitModal();
    // 移除原算子块，用子算子替换
    b.el.remove(); delete blocks[currentSplitBlock];
    let hwX=parseInt(b.el.style.left)||600, hwY=parseInt(b.el.style.top)||40;
    let baseX=hwX, baseY=hwY;
    d.kernels.forEach((nk,ni)=>{
      let nid='op'+(opCounter++);
      let el=document.createElement('div');
      el.className='block op-block';el.id=nid;el.style.left=(baseX+ni*250)+'px';el.style.top=(baseY+ni*120)+'px';
      let attrs=Object.entries(nk.attributes||{}).map(([a,v])=>a+'='+v).join(', ')||'—';
      let nIn=nk.inputs.length, nOut=nk.outputs.length;
      let inTop=(nIn>1?(di)=>25+di*50/(nIn-1):()=>50), outTop=(nOut>1?(di)=>25+di*50/(nOut-1):()=>50);
      let inPorts=nk.inputs.map((d,di)=>`<div class="port read" data-port="${nid}_in${di}" data-port-type="read" style="top:${inTop(di)}%" title="读: ${d}"><span class="plabel">读${di+1}</span></div>`).join('');
      let outPorts=nk.outputs.map((d,do_)=>`<div class="port write" data-port="${nid}_out${do_}" data-port-type="write" style="top:${outTop(do_)}%" title="写: ${d}"><span class="plabel">写${do_+1}</span></div>`).join('');
      let midPort=nk.intermediates.length?`<div class="port mid" data-port="${nid}_mid" data-port-type="mid" title="中间值: ${nk.intermediates.join(', ')}"><span class="plabel">中间值</span></div>`:'';
      el.innerHTML=`
        <div class="op-row"><span>类型</span><b>${nk.op_type||''}</b></div>
        <div class="op-row"><span>算子ID</span><span class="val">${nk.id||''}</span></div>
        <div class="op-row"><span>算子精度</span><span class="val p">${nk.precision||'FP16'}</span></div>
        <div class="op-row"><span>计算量</span><span class="val c">${nk.compute_gflops||'—'}</span></div>
        <div class="op-row"><span>内存需求</span><span class="val m">${nk.memory||'—'}</span></div>
        <div class="op-row"><span>形状/参数</span><span class="val">${attrs}</span></div>
        <div style="margin-top:6px;border-top:1px dashed #3a4050;padding-top:4px"><span class="split-btn" onclick="openSplitModal('${nid}')">✂ 切割</span></div>
        ${inPorts}${outPorts}${midPort}
      `;
      document.getElementById('canvas').appendChild(el);
      blocks[nid]={type:'operator',el:el,x:baseX+ni*250,y:baseY+ni*120,data:nk,kernelId:nk.id||nid,inputs:nk.inputs,outputs:nk.outputs,intermediates:nk.intermediates};
      el.querySelectorAll('.port').forEach(p=>p.addEventListener('mousedown',portMouseDown));
      makeDraggable(el);
    });
    updateStatus('算子 '+k.id+' 已沿 '+dim+' 切成 '+parts.length+' 段');
  }).catch(e=>updateStatus('切割失败: '+e));
}
