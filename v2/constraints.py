...
PRECISION_RANK = {"FP32": 4, "FP16": 3, "BF16": 3, "INT8": 2, "INT4": 1}
RANK_NAME = {4: "FP32", 3: "FP16", 2: "INT8", 1: "INT4"}
...
def _precision_rank(precision: Any) -> int:
    """算子要求的精度等级 → 数值等级（FP32=4 … INT4=1），未知返回 0。"""
...
class ConstraintChecker:
    ...
    def _rule_A_compat(self, o: dict, hw: dict):
        # A3 精度兼容：硬件支持的最高精度等级 >= 算子要求等级
        hw_max, _ = self._hw_precision(hw)
        op_rank = _precision_rank(o["precision"])
        if op_rank == 0:
            self._add("E4", "error", f"算子 {o['kernelId']} 精度 '{o['precision']}' 非法。", [o["id"]])
            return
        if hw_max < op_rank:
            self._add("A3", "error",
                      f"算子 {o['kernelId']} 要求 {o['precision']}(等级{op_rank})，"
    ...
    def _hw_precision(self, hw: dict):
        ...
        return (max(ranks) if ranks else 4), names
...
def validate_config(hardware=None, operators=None, connections=None,
                    compute_map=None, weight_blocks=None) -> ValidationResult:
