...
HARDWARE_CAPABILITY = {
    ...
    "RERAM_PIM": dict(
        # v3.1：ReRAM-PIM 只支持线性算子（存内计算以矩阵乘/线性类为主）。
        # 非线性算子（LN/Softmax/激活/Residual 等）改由 GPU / SRAM-PIM / CPU 承担，
        # 18 类算子仍有设备可跑（已扫描验证，无"无处可跑"的算子）。
        categories=[_L],
    ...
    # v3.1 新增：纯存储单元（只存不整）。categories/execution 为空 => 不执行任何算子。
    "SRAM": dict(
        categories=[],
        data=[PrecisionLevel.FP32, PrecisionLevel.BF16, PrecisionLevel.FP16,
              PrecisionLevel.FP8, PrecisionLevel.INT8, PrecisionLevel.INT4],
