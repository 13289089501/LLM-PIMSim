...
职责（"硬件"这一概念的完整闭环逻辑集中于此）：
  1. 硬件数据结构：HardwareUnit（能力+状态）、Link / Interconnect（非对称读写）
  2. 硬件 YAML 解析：HardwareConfig / LinkConfig + 出厂预设表 + 单位换算
  3. 硬件构建工厂：HardwareFactory（config → HardwareUnit / Interconnect）+ build_hardware 装配
...
def parse_interconnect(doc: dict, hardware: dict) -> list:
    """interconnect.yaml -> list[LinkConfig]（未写带宽用默认连接表补参考值）"""
    out = []
    links = doc.get("links", [])
...
class Link:
    ...
    def __init__(self, src: str, dst: str, read_bw: float, write_bw: float, latency_ns: int):
        self.src = src
        self.dst = dst
        self.read_bw_Bps = read_bw
        self.write_bw_Bps = write_bw
...
def build_hardware(cfg):
    """完整装配：硬件 + 互连。cfg 需含 .hardware(.dict)、.links。"""
