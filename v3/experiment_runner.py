...
def _construct_plan(exp_path, compute_map_override, weight_blocks, splits_override) -> dict:
    ...
    return {"wl": wl, "operators": operators, "data_objects": data_objects,
            "devices": devices, "interconnect": interconnect,
...
def validate_runnable(exp_path: str, compute_map_override: dict = None,
                      weight_blocks: list = None, splits_override: list = None) -> dict:
...
def run_workload_experiment(exp_path: str, compute_map_override: dict = None,
                            ...
                            splits_override: list = None) -> dict:
...
def _resolve_model_dims(ing) -> dict:
    ...
    dims = dict(MODEL_DIMS.get(ing.model, MODEL_DIMS["llama7b"]))
