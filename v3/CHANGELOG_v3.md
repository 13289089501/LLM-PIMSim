...
- **硬件数据结构**：`HardwareUnit` / `Link` / `Interconnect`（原 hardware.py）。
- **硬件解析**：`HardwareConfig` / `LinkConfig` / `parse_hardware` / `parse_interconnect` +
  出厂预设表（`DEFAULT_DEVICE_PARAMS` / `DEFAULT_CONNECT_TABLE`）+ 单位换算
  `throughput_to_flops`（自 config_loader 移入）。
- **构建工厂**：`HardwareFactory` / `build_hardware`（原 hardware_factory.py）。
