# LLM-PIMSim v3 — 配置驱动的 LLM×PIM 异构推理仿真器
...
v3 将原本混在一起的代码重构为 `core/` 下的 **8 大系统 + 1 个公共底层**，依赖方向单向、无循环：
...
| 硬件系统 | `hardware_sys.py` | 硬件/互连 YAML 解析、预设表、单位换算、HardwareUnit/Interconnect、硬件工厂 |
